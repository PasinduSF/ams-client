...
import {  authenticateAdmin, getNonce } from '@/api/apiAuth';
...
function Page() {
    ...
    const authenticate = async (account) => {
        try {
          ...
          const response = await authenticateAdmin(reqBody, tempToken);
          console.log('response', response);

          if (response.data && response.data.organizationId ) {
            localStorage.setItem('organizationId', response.data.organizationId);
            localStorage.setItem('adminWallet', response.data.adminWallet);
          }  
          router.push("/admin-dashboard?showMessage=true");
        } catch (error) {
